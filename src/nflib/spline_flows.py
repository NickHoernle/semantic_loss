...
class NSF_CL(nn.Module):
    ...
    def __init__(self, dim, K=5, B=3, hidden_dim=8, base_network=MLP, **kwargs):
        super().__init__()
        self.dim = dim
        self.K = K
        self.B = B
        ...
        self.f1 = base_network(dim // 2 + self.num_condition * self.condition, (3 * K - 1) * dim // 2, hidden_dim)
        self.f2 = base_network(dim // 2 + self.num_condition * self.condition, (3 * K - 1) * dim // 2, hidden_dim)
    ...
    def forward(self, x, **kwargs):
        ...
        if self.condition:
            lower_net = torch.cat((lower, kwargs['condition_variable']), -1)
            out = self.f1(lower_net).reshape(-1, self.dim // 2, 3 * self.K - 1)
        else:
        ...
        if self.condition:
            upper_net= torch.cat((upper, kwargs['condition_variable']), -1)
            out = self.f2(upper_net).reshape(-1, self.dim // 2, 3 * self.K - 1)
        else:
